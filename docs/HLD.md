...
```
                         Internet
                            │
                 CloudFront + WAF  (bot/rate/geo rules — V1 launch infra)
                            │
  SvelteKit UI / BFF ───────▼───────────────────────────────────────────
   │  POST /v1/oauth/token · /v1/otp/* · /v1/passkeys/* · GET /.well-known/jwks.json
   │                        │
   │              ┌─────────▼──────────┐
   │              │ auth-api  public   │ :7011   ECS Fargate via CDK (deploy/cdk/)
   │              └─────────┬──────────┘
   │                        │ shared network namespace (one image, two binaries)
   │              ┌─────────▼──────────┐
   │   VPC only ─▶│ auth-api  private  │ :7012   introspect · validate · client registry
   │              └─────────┬──────────┘
   │      ┌────────────┬────┴────────┬───────────────────┐
   │ Secrets Manager  ElastiCache   DynamoDB        HTTP adapters
   │ (JWT keyset,     (Redis: OTP,  (banned         ├─ user-api    (identity, passkey creds)
   │  client registry) challenges,   customers)     └─ comms-api   (OTP email delivery)
   │                   JTI denylist)
   │
   └── all other services: verify-only via forge-sdk JWKSVerifier; obtain their own
       M2M tokens via client_credentials (http/client.WithServiceAuth)
```
...
| `internal/clients` | Outbound adapters: comms-api (async bounded email), user-api (identity + passkey credentials), banned-customers (DynamoDB) |
...
All durable user data lives in user-api. Auth-api state is ephemeral, TTL'd Redis plus two SM secrets and one DynamoDB read — full schema in `DATA-MODEL.md`.
...
| user-api (private) | `USER#<id>` resolution, passkey credentials | OTP/passkey verify → 503 (lookup error) / 401 (no account) |
